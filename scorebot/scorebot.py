...
SANDBOX_RUNNER_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sandbox_run.py')

TEAM_LIST_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'team_list.json')
TEAM_LOCAL_USER_FORMAT = 'ctf-sandbox-team-%d'

LOG_PATH = '/var/log/ctf/scheduler.log'
...
class DBClient:
    ...
    def get_team_list(self):
        ...
        try:
            url = 'http://%s/getteamlist?secret=%s' % (self.host, self.pwd)

            r = urllib.urlopen(url).read()
            ret = json.loads(r)
            ret = [{'team_id': y['team_id'], 'ip': y['ip']} for _, y in ret.items()]

            self.log.info('Team list returned: %s' % (str(ret)))
            return ret
        except Exception as e:
            self.log.error(ERROR_DB[1] + ' get_team_list(): Exception: ' + str(e) + '. Response: ' + r)
    ...
    def push_result(self, team_id, script_id, result):
        ...
        try:
            ...
            self.log.error(ERROR_DB[1] + " push_result(team_id:%d,script_id:%d,result:%s):Exception:%s " % (
    ...
    def update_status_db(self, team_id, service_id, status, reason):
        ...
        if TEST:
            return {
                'scripts': [{'id': 3, 'type': 'benign', 'service_id': 1, 'call_interval': 5, 'call_interval_sd': 3}]}
        try:
            ...
            ret = json.loads(urllib.urlopen(url).read())
            self.log.info(
                'update_status_db(%d, %d, %d, %s) returned %s.' % (team_id, service_id, status, reason, str(ret)))
            return ret
        except Exception as e:
            self.log.error(ERROR_DB[1] + " update_status_db(%s): Excpetion: %s" % (
...
class ScriptExec(Process):
    def __init__(self, slock, team_id, sandbox_user, script_id, service_id, service_name, timeout, script_type,
        ...
        self.out_status = (Value('i', 0), Array('c',
                                                '\0' * 1024))  # thread-safe replica of self.result, which is readable from parent process.
    ...
    def get_args(self):
        try:
            ...
            msg = ERROR_SCRIPT_EXECUTION[1] + " Exception at ScriptExec.get_args(): " + str(
                e) + ' | Script Object:' + str(self.get_status())
            self.log.error(msg)
    ...
    def push_result(self, result):
        try:
            ...
            result = {'ERROR': ERROR_SCRIPT_EXECUTION[0],
                      'ERROR_MSG': ERROR_SCRIPT_EXECUTION[1] + " Exception at ScriptExec.push_result(): " + str(
                          e) + ' | Script Object:' + str(self.get_status())}
    ...
    def get_status(self):
        s = {'pid': self.pid, 'exitcode': self.exitcode, 'error': int(self.out_status[0].value),
             'error_msg': str(self.out_status[1].value), 'team_id': self.team_id, 'scritp_id': self.script_id,
             'script_type': self.script_type, 'dest_ip': self.ip, 'dest_port': self.port, 'delay': self.delay,
    ...
    def run(self):
        ...
        self.log.info('Running script_id:%d, type:%s, dest_ip:%s, dest_port:%d, delay:%.2f secs.' % (
        self.script_id, self.script_type, self.ip, self.port, self.delay))
        ...
        if TEST:
            ...
            self.result = {'ERROR': 0, 'ERROR_MSG': 'Success', 'FLAG': 'this_is_a_flag', 'FLAG_ID': 23,
                           'TOKEN': 'this_is_a_cookies'}
        ...
        while True:
            ...
            try:
                ...
                if self.args:
                    ...
                    if self.result['ERROR'] != 0:
                        self.result["ERROR_MSG"] = self.result["ERROR_MSG"] + ' | Script Object:' + str(
                            self.get_status()) + " | Script output:" + output + err
                ...
                if output is None:
                    output = ''
                self.result = {'ERROR': ERROR_SCRIPT_EXECUTION[0],
                               'ERROR_MSG': ERROR_SCRIPT_EXECUTION[1] + " Exception at ScriptExec.run(): " + str(
                                   e) + ' | Script Object:' + str(
                                   self.get_status()) + " | Script output:" + output + err}
...
class Scheduler:
    def __init__(self, team_list_path=TEAM_LIST_PATH, status_path=STATUS_PATH):
        ...
        self.status = {'state_id': 'INIT', 'last_error': None, 'script_err': None, 'script_ok': 0, 'script_fail': 0,
                       'script_tot': 0}
            ...
            if stype == 'getflag':
                if last_delay is not None:
                    if (delay - last_delay) < SET_GET_FLAG_TIME_DIFFERENCE_MIN:
                        self.log.info(
                            'delay (%.2f) - last_delay (%.2f) < SET_GET_FLAG_TIME_DIFFERENCE_MIN (%.2f)' % (delay,
                                                                                                            last_delay,
        ...
        for srvid, slist in ss.iteritems():
            ...
            for sid, delay in rlist:
                ...
                if p is None:
                    continue
                s = self.scripts[sid]
                self.runscript(slock, team['team_id'], sid, s['service_id'], SCRIPT_TIMEOUT, s['type'], p, ip,
                               self.services[s['service_id']]['port'], delay)
    ...
    def setuprun(self, team):
        ...
        for sid, s in self.scripts.iteritems():
            if s['type'] == 'setflag':
                ...
                self.runscript(self.status, team['team_id'], sid, s['service_id'], SCRIPT_TIMEOUT, s['type'], p, ip,
                               self.services[s['service_id']]['port'])
...
def main():
    ...
    dbc = DBClient()
    team_list = dbc.get_team_list()
    with open(TEAM_LIST_PATH, "w") as fp:
        json.dump(team_list, fp)

    print '[+] Teamlist loaded from API'
